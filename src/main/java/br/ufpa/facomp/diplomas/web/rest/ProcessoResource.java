...
/**
 * REST controller for managing {@link br.ufpa.facomp.diplomas.domain.Processo}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Processo> partialUpdateProcesso(
        ...
        Optional<Processo> result = processoRepository
            ...
            .map(existingProcesso -> {
                if (processo.getSipac() != null) {
                    existingProcesso.setSipac(processo.getSipac());
                }
                if (processo.getInicio() != null) {
                    existingProcesso.setInicio(processo.getInicio());
                }
                if (processo.getDefesa() != null) {
                    existingProcesso.setDefesa(processo.getDefesa());
                }
                if (processo.getEnviadoBiblioteca() != null) {
                    existingProcesso.setEnviadoBiblioteca(processo.getEnviadoBiblioteca());
                }
                if (processo.getEntregaDiploma() != null) {
                    existingProcesso.setEntregaDiploma(processo.getEntregaDiploma());
                }
